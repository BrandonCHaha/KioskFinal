...
namespace KioskMainProject {
    internal class Program {
        ...
        public struct Bank {
            public void ChangeMaker(double change) {
                ...
                for (int i = 0; i < _cashValue.Length; i++) {
                    if ((_change % _cashValue[i]) < _change) {
                        if (_change / _cashValue[i] <= _cashDrawer[i]) {
                            ...
                            if (i == 9) {
                                _change = Math.Round( _change, 2);
                                Console.WriteLine($"${_change} left. This machine is out of money please contact an employee or pay another way.");
